...
# ---------------------------------------------------------------------
# CIRA mask of feature constraints
# ---------------------------------------------------------------------
...
CIRA_ALLOWED_FEATURES_EXTENDED = CIRA_TRAFFIC_FEATURES.copy()

CIRA_ALLOWED_FEATURES_BY_MODE = {
...
def build_cira_feature_mask(
    feature_cols: Iterable[str],
    mask_mode: str = "operational",
) -> Tuple[np.ndarray, pd.Series]:
    """Builds the CIRA mask of feature constraints."""
    feature_cols = list(feature_cols)
    mask_mode = str(mask_mode).lower().strip()

    if mask_mode not in CIRA_ALLOWED_FEATURES_BY_MODE:
        raise ValueError(
            f"Unknown mask_mode={mask_mode!r}. "
            f"Use one of: {sorted(CIRA_ALLOWED_FEATURES_BY_MODE)}"
        )

    leaked_identifiers = [
        column for column in CIRA_IDENTIFIER_COLUMNS if column in feature_cols
    ]
    leaked_targets = [
        column for column in CIRA_TARGET_CANDIDATES if column in feature_cols
    ]

    if leaked_identifiers:
        raise ValueError(
            f"Identifier columns found in model features: {leaked_identifiers}"
        )

    if leaked_targets:
        raise ValueError(
            f"Target columns found in model features: {leaked_targets}"
        )
    ...
    allowed_features = CIRA_ALLOWED_FEATURES_BY_MODE[mask_mode]
    missing_allowed = [
        feature for feature in allowed_features if feature not in feature_cols
    ]

    if missing_allowed:
        raise ValueError(
            "The following perturbable CIRA features were not found in X: "
            f"{missing_allowed}"
        )
...
def print_cira_mask_report(mask_series: pd.Series, mask_mode: str) -> None:
    """Prints the active CIRA mask of feature constraints."""
    allowed = mask_series[mask_series == 1.0]
    blocked = mask_series[mask_series == 0.0]

    print("\n" + "-" * 78)
    print(f"[CIRA mask of feature constraints] mode={mask_mode}")
...
DATA_PIPELINE_PROTOCOL_VERSION = "CIRA_MEMMAP_BATCH_STREAMING_V1"
MODEL_PROTOCOL_VERSION = "M1_3CONV_2ECA_TRANSFORMER_4TOKENS_DROPOUT64_CIRA_MASKED_V5"
...
def get_data_cache_root(args: argparse.Namespace) -> Path:
    ...
    return Path(args.csv).expanduser().resolve().parent / ".cira_ram_cache"
...
def _common_cache_key_payload(args: argparse.Namespace) -> Dict[str, Any]:
    return {
        ...
        "label_col": int(args.label_col),
        "legacy_feature_slice": bool(args.legacy_feature_slice),
        "expected_features": CIRA_EXPECTED_FEATURES,
        "cleaning": [
            "remove_strong_identifiers",
            "map_final_target_to_binary",
...
def ensure_common_data_cache(args: argparse.Namespace) -> CommonDataCache:
    """Creates the cleaned CIRA matrix once and reuses it across workers."""
    ...
    with exclusive_file_lock(lock_path):
        ...
        print("[Data cache] Building the shared cleaned CIRA cache once.")
...
def build_mtl_model_M1(input_shape: Tuple[int, int, int], learning_rate: float = 1e-4) -> Model:
    """Builds M1 with three CNN blocks, ECA-Net, and a Transformer encoder."""
    ...
    # Convolutional Block 1: 6x6 -> 3x3 for the standard CIRA input.
    ...
    # Convolutional Block 2: 3x3 -> 2x2. The same padding preserves enough
    # spatial positions for a meaningful Transformer sequence.
    ...
    # Convolutional Block 3: keep the 2x2 spatial grid. No pooling is applied
    # here, so the Transformer receives four tokens instead of one.
...
def load_and_clean_csv(csv_path: str) -> pd.DataFrame:
    """Loads and cleans the CIRA-CIC-DoHBrw-2020 CSV.

    Strong identifiers are removed, the final target column is mapped to a
    binary label, and feature columns are converted to numeric values. Numerical
    precision is preserved until SMOTE and MinMaxScaler have completed.
    """
    start = time.perf_counter()
    df = pd.read_csv(csv_path)

    columns_to_drop = [
        column for column in CIRA_IDENTIFIER_COLUMNS if column in df.columns
    ]
    if columns_to_drop:
        df = df.drop(columns=columns_to_drop)
        print(f"[PREP] Strong identifiers removed: {columns_to_drop}")

    if df.shape[1] < 2:
        raise ValueError(
            "The CIRA dataframe must contain traffic features and a target column."
        )
    ...
    print(
        f"[LOAD] Samples after cleaning: {len(df):,} | "
        f"columns: {df.shape[1]} | time={elapsed:.2f}s"
    )
    print(
        "[LOAD] Label distribution: "
        f"{dict(df[label_column].value_counts().sort_index())}"
    )
    return df
...
def select_features_and_label(
    ...
    """Selects the 29 CIRA traffic features and the binary target."""
    df_local = df.copy()

    y = df_local.iloc[:, label_col].astype("int32").copy()
    target_column = df_local.columns[label_col]

    if legacy_feature_slice:
        end = -feature_end_offset if feature_end_offset > 0 else None
        X = df_local.iloc[:, feature_start_col:end].copy()
        print(
            "[Warning] Using legacy iloc slice selection. This may be "
            "incompatible with the named CIRA feature mask."
        )
    else:
        named_features = [
            column
            for column in CIRA_TRAFFIC_FEATURES
            if column in df_local.columns
        ]

        if len(named_features) == CIRA_EXPECTED_FEATURES:
            X = df_local.loc[:, named_features].copy()
        else:
            blocked_columns = set(
                CIRA_IDENTIFIER_COLUMNS
                + CIRA_TARGET_CANDIDATES
                + [target_column]
            )
            candidate_columns = [
                column
            ...
            numeric_columns = [
                ...
                if pd.api.types.is_numeric_dtype(df_local[column])
            ]

            if len(numeric_columns) != CIRA_EXPECTED_FEATURES:
                raise ValueError(
                    "Could not identify the 29 CIRA traffic features. "
                    f"Found {len(named_features)} named CIRA features and "
                    f"{len(numeric_columns)} numeric candidate features. "
                    "Check the column names or use --legacy-feature-slice "
                    "only when the selected columns match the intended set."
                )

            X = df_local.loc[:, numeric_columns].copy()

    leaked_identifiers = [
        column for column in CIRA_IDENTIFIER_COLUMNS if column in X.columns
    ]
    leaked_targets = [
        column for column in CIRA_TARGET_CANDIDATES if column in X.columns
    ]

    if leaked_identifiers:
        raise ValueError(
            f"Identifier columns leaked into X: {leaked_identifiers}"
        )
    if leaked_targets:
        raise ValueError(f"Target columns leaked into X: {leaked_targets}")
    ...
    if X.shape[1] != CIRA_EXPECTED_FEATURES:
        raise ValueError(
            f"Expected {CIRA_EXPECTED_FEATURES} CIRA traffic features in X, "
            f"but received {X.shape[1]}."
        )
    if y.nunique() != 2:
        print(
            f"[Warning] Label has {y.nunique()} classes. "
            "The script assumes binary classification."
        )
...
def latex_mean_std_table(summary: pd.DataFrame, metric_names: Optional[List[str]] = None) -> str:
    ...
    lines.append("\\caption{Adversarial training results on CIRA-CIC-DoHBrw-2020. Values are reported as mean $\\pm$ standard deviation.}")
    lines.append("\\label{tab:cira_adv_training_results}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Adversarial training of the M1 model on CIRA-CIC-DoHBrw-2020 with a "
            "feature-constraint mask, per-epoch checkpoint resumption, and "
            "memory-mapped batch streaming."
        )
    )
    parser.add_argument("--csv", required=True, help="Path to the CIRA-CIC-DoHBrw-2020 CSV file.")
    parser.add_argument(
        "--output-dir",
        default="results_cira_adversarial_training_masked",
    ...
    parser.add_argument(
        "--mask-mode",
        choices=sorted(CIRA_ALLOWED_FEATURES_BY_MODE.keys()),
        default="operational",
        help="CIRA feature-constraint mask mode. Default: operational.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<dataset-directory>/.cira_ram_cache. All concurrent workers "
...
def main() -> None:
    ...
    feature_mask_1d, mask_series = build_cira_feature_mask(
        feature_cols,
        args.mask_mode,
    )
    print_cira_mask_report(mask_series, args.mask_mode)

    mask_csv_path = output_dir / "cira_mask_of_feature_constraints.csv"
